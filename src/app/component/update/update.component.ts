...
export class UpdateComponent implements OnInit {
  ...
  check_update() {
    this._updateService
      .check_release(this.git_username, this.git_repo_name)
      .subscribe(http_data => {
        if (http_data) {
          ...
          if (arr_curr_version.length === 3 && arr_new_version.length === 3) {
            if (
              parseInt(arr_new_version[0], 10) >
              parseInt(arr_curr_version[0], 10)
            ) {
              new_version_available = true;
            } else if (
              parseInt(arr_new_version[1], 10) >
              parseInt(arr_curr_version[1], 10)
            ) {
              new_version_available = true;
            } else if (
              parseInt(arr_new_version[2], 10) >
              parseInt(arr_curr_version[2], 10)
            ) {
              new_version_available = true;
            } else {
